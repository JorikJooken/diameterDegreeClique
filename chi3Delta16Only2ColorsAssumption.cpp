...
void recursivelyGeneratePartitions(vector<int> &nbInColorClass, int idx, int sum)
{
    if(idx==chi)
    {
        ...
        for(int i=0; i<chi; i++)
        {
            if(nbInColorClass[i]>0) nbNonZero++;
        }
        if(nbNonZero==chi) return;
        graph g;
...
//vector< map<state, pair<state, edgeSetBetweenConsecutiveLayers> > > previousState;
    ...
    {
        if(allGraphs[firstLayerID].nbInColorClass[1] != 0 || allGraphs[firstLayerID].nbInColorClass[2] != 0)
        {
            continue;
        }
        int nFirstLayer=0;
        ...
        {
            if(allGraphs[secondLayerID].nbInColorClass[1]>allGraphs[secondLayerID].nbInColorClass[2])
            {
                continue;
            }
            int nSecondLayer=0;
            ...
            /*if(delta==4 && nFirstLayer+nSecondLayer>=4) continue;
            if(delta==5 && nFirstLayer+nSecondLayer>=5) continue;
            if(delta==6 && nFirstLayer+nSecondLayer>=6) continue;
            if(delta==7 && nFirstLayer+nSecondLayer>=6) continue;
            if(delta==8 && nFirstLayer+nSecondLayer>=7) continue;
            if(delta==9 && nFirstLayer+nSecondLayer>=8) continue;
            if(delta==10 && nFirstLayer+nSecondLayer>=9) continue;
            if(delta==11 && nFirstLayer+nSecondLayer>=10) continue;
            if(delta==12 && nFirstLayer+nSecondLayer>=11) continue;
            if(delta==13 && nFirstLayer+nSecondLayer>=12) continue;
            if(delta==14 && nFirstLayer+nSecondLayer>=12) continue;
            if(delta==15 && nFirstLayer+nSecondLayer>=13) continue;
            if(delta==16 && nFirstLayer+nSecondLayer>=14) continue;*/
            caseNumber+=1;
            //continue;
    ...
    cerr << "number cases: " << caseNumber << endl;
